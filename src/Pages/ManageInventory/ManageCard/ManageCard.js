...
const ManageCard = ({ product }) => {
    ...
    return (
        <li className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 my-5 justify-items-center content-center items-center'>
            ...
            <div><button onClick={handleDelete} className='bg-red-500 hover:bg-red-700 duration-500 text-slate-200  py-2 px-7 rounded-md'>Delete</button></div>
            <div><Link to={`/inventory/${_id}`} className='bg-blue-500 hover:bg-blue-700 duration-500  text-slate-200 py-2 px-7 rounded-md'>Update</Link></div>
        </li>
    );
};
